...
public class sizeAnimator : MonoBehaviour
{
    ...
    public void AnimateSize()
    {
        ...
        if (!isUsingRect)
        {
            sequence.Append(transform.DOScale(maxSize, timeToNormal).SetEase(maxEase))
                .Append(transform.DOScale(defaultSize, timeToNormal).SetEase(normalEase));
        }
        else
        {
            var rect = GetComponent<RectTransform>();
            sequence.Append(DOTween.To(() => rect.localScale, x => rect.localScale = x, maxSize, timeToMax).SetEase(maxEase))
